...
let raw = data
let provinces = raw.map(d => d.name)          //遍历后获取省份名称
let currentProvince = provinces[0]
let initCities = sub(raw[0])                  // 获取到第一个省份下的所有的市
let currentCity = initCities[0]
let initDistricts = getDistricts(currentProvince, currentCity)        // 获取当前省份，市下的所有区
...
export default {
	...
	init(id, opts = {}) {
		const that = this
		const updateValue = (cols = [], arrValues = []) => {         // 当选择不同的地点时
			let newValue = []
			let newValueIndex = []
			let newDisplayValue = []
			...
			for (let i = 0; i < cols.length; i++) {
				if (cols[i]) {                                              // 新的当前列有值
					const values = cols[i].values || []                     // 给当前的新的值
					const displayValue = cols[i].displayValue || []
					const valueIndex = arrValues[i]                          // 当前选中的三个地点的序列号

					newValueIndex.push( typeof values[valueIndex] !== `undefined` ? valueIndex : 0)          // 如果选中的有值 则赋值
